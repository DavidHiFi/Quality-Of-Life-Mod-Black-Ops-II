...
| Who's Who fx | 🌟 v1.99.16 (deployed, not yet confirmed). Full audit of every effect stock gives the perk: **ghost-state colour grade** — all 23 entries of Die Rise's `zm_whos_who.vision` driven through the `vc_*` dvars (confirmed working); **audio** — sting, looper and mixer snapshot (confirmed working); **red downed screen** — stock's `visionsetlaststand( "zombie_last_stand" )`, which v1.99.14–15 suppressed because `r_filmUseTweaks 1` overrides visionsets; the grade now waits until the player leaves last stand, so the red plays first exactly as it does on Die Rise; **clone glow** — the corpse is only an *actor* on maps shipping a `fake_player_spawner` entity (`mapents` dump: Die Rise 1, TranZit 0, Origins 0), so the stock `actor` clientfield reached nothing; a `scriptmover` twin now carries it, using stock's own callback; **screen filter** — `generic_filter_afterlife` and its assets ship since v1.99.13. The shellshock is deliberately absent: it is gated on `level.chugabud_shellshock`, which stock sets nowhere. |
