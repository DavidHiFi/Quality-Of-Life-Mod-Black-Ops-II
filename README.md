...
| Winter's Howl firing effects | Root cause found in v1.99.7, fix deployed but **not yet confirmed in game**. Only 1 of the gun's 11 muzzle-flash elements was flagged to draw with the viewmodel, so the other ten rendered in the world pass at the camera and were invisible — and the one that did draw is a heat-shimmer distortion, which is exactly the "weird wind" that showed instead of a flash. The Wunderwaffe (14 of 14 flagged) has always looked right and the Thundergun (8 of 14) partly right. v1.99.7 flags the six remaining visible elements. |
