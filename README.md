...
| Who's Who has no screen overlay | 🌟 v1.99.14 — the cause is this mod's own **night mode**. It sets `r_filmUseTweaks 1` plus `vc_rgbh/vc_yl/vc_yh/vc_rgbl`, which are the same colour-grade parameters a `.vision` file carries, so the renderer uses the dvars *instead of* the active visionset. The visionset activated correctly and was ignored. The overlay is now driven through those same dvars, with the values read out of Die Rise's own `zm_whos_who.vision`, and restored to night-mode or neutral on revive. The v1.99.13 asset fix was necessary but not sufficient. **Deployed, not yet confirmed in game.** |
