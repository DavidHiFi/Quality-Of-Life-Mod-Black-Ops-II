...
| Winter's Howl firing effects | **Muzzle flash fixed and confirmed in game (v1.99.7).** Only 1 of the gun's 11 muzzle-flash elements was flagged to draw with the viewmodel, so the rest rendered in the world pass at the camera and were invisible; v1.99.7 flags the six remaining visible ones. The Wunderwaffe (14 of 14 flagged) always looked right and the Thundergun (8 of 14) partly right. The separate "wind" the gun also showed was **not** the heat-shimmer element — v1.99.9 disabled that on a wrong theory and the boot proved it is part of the correct flash, so it was restored. v1.99.10 then removed `fx_freezegun_smoke_cloud` on the same reasoning and that was wrong too — the boot showed the gun drawing nothing at all, so that cloud IS the white frost burst, not a stray "wind". v1.99.11 restores it. The gun is back to the confirmed-good v1.99.7 state. **Deployed, not yet confirmed in game.** |
| Zombie risers are silent | 🌟 ROOT-CAUSED AND FIXED in v1.99.11 (deployed, not yet confirmed in game). Stock `zmb_zombie_spawn` names its payloads `dirt_00/01.LN55.pc.snd` with `Storage=loaded`, and that audio data ships in **no bank at all** — OpenAssetTools reports `Could not find data for sound` for both, in every bank that defines the alias. The alias resolves and plays silence, which is exactly what `.testsound` measured. The same dirt audio DOES ship, streamed, as `dirt_00/01.SN50.pc.snd.flac` under Origins' `evt_zombie_dig_dirt`. The mod now ships that payload in `mod.all` under `zmqol_zombie_riser` — `zmb_zombie_spawn`'s own alias row verbatim, repointed. |
