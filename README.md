...
| **MOTD Galaxy Camo Animated** *(community texture pack; no author named in the download)* | The three textures behind the **animated Pack-a-Punch camo** — `camo_zmb_dlc2_ember`, `_heat` and `_reveal`, which are what the Mob of the Dead camo animates with. The pack ships them as a loose drop into `t6r\data\images`, where this mod's own copy of those names would silently win; they are built into `mod.ff` and `mod.iwd` here instead so the galaxy look actually reaches the gun. |
