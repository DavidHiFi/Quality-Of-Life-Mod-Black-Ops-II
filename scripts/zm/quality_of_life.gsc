...
vpa_onplayerspawned()
{
    ...
    for ( ;; )
    {
        ...
        //  v1.61.0 - the mod-drawn perk icon row. Started here because this
        //  block already runs once per spawn for this player, and the row must
        //  be rebuilt after a respawn: newclienthudelem elements do not survive
        //  one, so the old handles are dead and the signature must re-evaluate
        //  from scratch. The loop notifies its own name first, so a second
        //  start replaces the first rather than doubling the row.
        self thread zmqol_perk_hud_think();
    }
}
...
// ============================================================================
//  zmqol_perk_hud_think  -  THE PERK ICON ROW, drawn by the mod.
//
//  v1.61.0. The engine's own perk-icon row rendered every perk as PhD
//  Flopper and no script change could reach it - see the block in
//  zm_expanded.csc::perks_register_clientfield for the full elimination.
//  Engine icons are now off and this draws the row instead.
//
//  🌟 WHY THIS CANNOT REPRODUCE THE BUG. The engine version looked its icon
//  up through a clientfield/version table this mod does not control. This
//  version reads hasperk() and calls getperkshader() - both plain script,
//  both already verified correct (12 perks, 12 distinct shaders). There is no
//  lookup left that can return the wrong icon, and no shared index to collide.
//
//  Rebuilt only when the perk SET changes, detected by comparing a signature
//  string. Polling and re-creating hud elements every frame is the reliable
//  command flood this project's notes warn about.
//
//  🛑 HUD ELEMENT BUDGET. A client's allowance is finite and this mod already
//  spends ~13 - overrunning it is what silently truncated the .help panel. So
//  an element is created ONLY for a perk actually held (max 12, and only if
//  the player really has all twelve), every element is destroyed on every
//  rebuild, and the array is cleared so nothing can leak across rebuilds. That
//  leak is exactly what left twelve stale icons on screen after .removeperks.
//
//  Order is fixed and map-independent, so the row does not reshuffle as perks
//  are gained.
// ============================================================================
zmqol_perk_hud_order()
{
    a = [];
    a[a.size] = "specialty_armorvest";                  // Juggernog
    a[a.size] = "specialty_quickrevive";                // Quick Revive
    a[a.size] = "specialty_fastreload";                 // Speed Cola
    a[a.size] = "specialty_rof";                        // Double Tap
    a[a.size] = "specialty_longersprint";               // Stamin-Up
    a[a.size] = "specialty_additionalprimaryweapon";    // Mule Kick
    a[a.size] = "specialty_deadshot";                   // Deadshot
    a[a.size] = "specialty_flakjacket";                 // PhD Flopper
    a[a.size] = "specialty_scavenger";                  // Tombstone
    a[a.size] = "specialty_finalstand";                 // Who's Who
    a[a.size] = "specialty_grenadepulldeath";           // Electric Cherry
    a[a.size] = "specialty_nomotionsensor";             // Vulture Aid
    return a;
}

zmqol_perk_hud_signature()
{
    a = zmqol_perk_hud_order();
    str = "";
...
zmqol_perk_hud_clear()
{
    if ( !isdefined( self.zmqol_perkicons ) )
        return;

    for ( i = 0; i < self.zmqol_perkicons.size; i++ )
    {
        if ( isdefined( self.zmqol_perkicons[i] ) )
            self.zmqol_perkicons[i] destroy();
    }

    self.zmqol_perkicons = [];
}

zmqol_perk_hud_build()
{
    self zmqol_perk_hud_clear();
...
zmqol_perk_hud_think()
{
    //  🛑 ONE INSTANCE PER PLAYER. This starts on every "spawned_player", so a
    //  respawn would otherwise leave two loops both building rows and the
    //  icons would double. Same notify/endon pattern prone_bonus_monitor uses:
    //  the notify kills any previous copy before this one arms its endon.
    self notify( "zmqol_perk_hud_think" );
    self endon( "zmqol_perk_hud_think" );
    self endon( "disconnect" );
    level endon( "end_game" );

    //  Elements do not survive a respawn, so drop any stale handles and force
    //  the first comparison to rebuild.
    self zmqol_perk_hud_clear();
    ...
    for ( ;; )
    {
        //  Poll rather than hook: perks change through the machines, the
        //  Wunderfizz, .giveperks, .removeperks, downs, Who's Who revives and
        //  Tombstone pickups. Watching the RESULT catches every one of those
        //  without a hook per path - and a missed hook is how stale icons got
        //  left on screen in the first place.
        str_now = self zmqol_perk_hud_signature();
