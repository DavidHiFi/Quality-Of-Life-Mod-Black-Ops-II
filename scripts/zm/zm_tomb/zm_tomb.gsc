...
main()
{
    ...
    //  v1.99.39 - remove the bunker Five-seven wall-buy. See the function for
    //  the evidence that it is STOCK Origins content, not something this mod
    //  adds, and for why the client half has to run too.
...
// ============================================================================
//  zmqol_tomb_remove_fiveseven_wallbuy  -  the bunker Five-seven wall-buy, gone
//
//  User, 2026-08-17, with a screenshot from the bunker between generators 2 and
//  3 on the way to No Man's Land: *"there was this random unnatural five-seven
//  weapon wallbuy, this is not normal and not apart of origins, and is being
//  added via my mod, get rid of that"*.
//
//  🛑 IT IS STOCK ORIGINS, AND THAT WAS SETTLED BEFORE A LINE WAS WRITTEN.
//  Removing it is still exactly what was asked for - this note exists so the
//  reason is on the record and nobody re-investigates it later:
//
//    1. `Unlinker --include-assets mapents` on the RETAIL zone\all\zm_tomb.ff
//       has two `zombie_weapon_upgrade "fiveseven_zm"` structs, at
//       (1270.64, -2593.29, 146.5) and (-927.75, 3036, -52). Neither carries a
//       `script_noteworthy`, and _zm_weapons.gsc:872 spawns every wall-buy
//       struct that has none, in every gametype.
//    2. Stock zm_tomb.gsc:1025 registers it itself:
//         add_zombie_weapon( "fiveseven_zm", "fiveseven_upgraded_zm",
//                            &"ZOMBIE_WEAPON_FIVESEVEN", 1100, "wpck_pistol" ... )
//       1100 is the cost printed in the user's own screenshot.
//    3. 🌟 THE DECIDER: a RUNTIME clientfield dump of an UNMODDED Origins,
//       Black Ops 2 Grand Resources\T6-Data-Archive-main\ZM\Clientfields\
//       Standard Gametype\Standard Location\clientfields_zm_tomb_zclassic_tomb.txt
//       lines 55-56, registers
//         world fiveseven_zm_(1270.64, -2593.29, 146.5)
//         world fiveseven_zm_(-927.75, 3036, -52)
//       A clientfield only exists for a wall-buy the game actually built. The
//       stock game builds both. This mod never touched either.
//
//  📝 ONLY THE ONE THE USER POINTED AT IS REMOVED. They said "simply remove that
//  wallbuy only", so the other Five-seven (1270.64, -2593.29, 146.5) is left
//  alone; flip STR_ORIGIN below to remove that one too if they ask.
//
//  🛑 WHY THE CLIENTFIELD REGISTRATION IS DELIBERATELY LEFT ALONE.
//  The tempting fix is to stop the struct reaching the spawn list - stock's own
//  `script_noteworthy` gametype filter would do it in one line. It cannot be
//  done here, twice over:
//    * server and client each build the list from their OWN copy of the structs
//      (_zm_weapons.gsc:842 and _zm_weapons.csc:175) and each registers the
//      clientfield from it. Editing one VM's structs and not the other's is an
//      EXE_CLIENT_FIELD_MISMATCH at load - a dead boot, not a missing feature.
//    * this mod's main() runs BEFORE the map's, so level.struct is still empty
//      here anyway. That is not a guess: v1.59.1 tried it for the MP40 and its
//      own probe printed "retagged 0 of 0 weapon_upgrade struct(s)".
//  So both sides register exactly what stock registers, symmetry is untouched,
//  and the wall-buy is dismantled AFTERWARDS - the trigger here, the chalk fx
//  and the gun model in zm_tomb.csc.
//
//  What has to go, all three or the result is the "chalk with no prompt" bug
//  this map has already had once:
//      the buy trigger  -> _zm_unitrigger::unregister_unitrigger (this file)
//      the gun model    -> its world clientfield forced to 0 (this file);
//                          the client also hides it directly, belt and braces
//      the chalk fx     -> stopfx, client only (zm_tomb.csc)
// ============================================================================
zmqol_tomb_remove_fiveseven_wallbuy()
{
    level endon( "end_game" );

    //  The struct origin from the mapents dump. The unitrigger stub is pushed
    //  off it by up to script_length * 0.4 (_zm_weapons.gsc:944), so this is a
    //  proximity test, never an equality test.
    v_target = ( -927.75, 3036, -52 );

    n_passes = 0;
    n_gone   = 0;

    //  Same shape as zmqol_mp40_keep_wallbuys_stalker: re-scan rather than bet
    //  on a window, because wall-buy stubs have been observed registering late
    //  on this map. Stops as soon as it has removed one.
    while ( n_passes < 120 && n_gone == 0 )
    {
        n_passes++;
        ...
        if ( isdefined( level._unitriggers ) && isdefined( level._unitriggers.trigger_stubs ) )
        {
            ...
            for ( i = 0; i < a_stubs.size; i++ )
            {
                ...
                if ( a_stubs[i].zombie_weapon_upgrade != "fiveseven_zm" )
                    continue;

                if ( !isdefined( a_stubs[i].origin ) )
                    continue;

                if ( distancesquared( a_stubs[i].origin, v_target ) > 4096 )   // 64 units
                    continue;

                //  Hide the wall model first. Stock only ever sets this field to
                //  1 from show_all_weapon_buys(), which is reached from the
                //  trigger's own think - so once the trigger is gone nothing can
                //  set it back. Writing 0 covers the case where the player had
                //  already walked past it before this pass ran.
    ...
    //  Loud when it fails, silent-by-omission never again. If this line shows up
    //  the trigger is still live and the cause is upstream of the stub list.
    if ( n_gone == 0 )
        println( "[zm_qol] origins fiveseven: NO matching wall-buy stub found in 60s - the trigger is still there" );
}
