...
perks_register_clientfield()
{
	...
	level thread zmqol_perk_init_code_callbacks();
}

// ============================================================================
//  zmqol_perk_init_code_callbacks  -  PROBE. Stock's perk_init_code_callbacks
//  body, copied exactly, with a println per call. NOTHING behavioural changes.
//
//  Why: every perk icon renders as PhD Flopper. The server side is already
//  EXONERATED - the v1.59.8 probe logged 158 set_perk_clientfield calls and
//  every one was correct, and getperkshader() maps all 12 perks to 12 distinct
//  shaders. So the fault is in the engine-drawn HUD, and the only thing script
//  controls there is WHICH fields get handed to setupclientfieldcodecallbacks
//  and in what ORDER.
//
//  setupclientfieldcodecallbacks is what tells the engine "this field drives a
//  perk icon". If a field is handed over twice, or the custom-perk loop hands
//  over the same one repeatedly, the engine can end up drawing one icon for
//  everything - which is exactly the symptom.
//
//  The log will show one line per registration, in order. Read it back and the
//  answer is immediate: perk_dive_to_nuke appearing more than once, or
//  appearing where another perk should be, names the bug outright.
// ============================================================================
zmqol_perk_init_code_callbacks()
{
	wait 0.1;
	...
	if (level._custom_perks.size > 0)
	{
		...
		for (i = 0; i < a_keys.size; i++)
		{
			...
			{
				println("[zm_qol] perkcode: CUSTOM " + a_keys[i]);
