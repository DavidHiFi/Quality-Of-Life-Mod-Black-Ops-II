...
### 🛑 B-STALEGSC — `mod.ff` RUNS A PRE-MERGE COPY OF THE MOD'S OWN SCRIPT, ON EVERY MAP

Found 2026-08-16 while chasing B-RISERSND. **This is a live landmine, not a tidiness item.**

`console_zm.log` carries four of these at every map load:
...
```
…and the same for `_zm_perks::perks_register_clientfield`, `::give_perk`, `::default_vending_precaching`.

**`scripts/zm/zm_expanded.gsc` DOES NOT EXIST IN THIS PROJECT.** It was the pre-merge module that became
part of `quality_of_life.gsc`. But `zone_source\mod_base.zone` (the donor's inventory) still declares
it, so **`mod.ff` ships the donor's day-one copy and Plutonium executes it**. The link output says so
in one line:

```
Loaded script "scripts/zm/zm_expanded.csc" (src: disk)   <- ours, staged
Loaded script "scripts/zm/zm_expanded.gsc" (src: mod)    <- the DONOR'S, 542 lines, from July
```

🛑 **`CLAUDE.md` §8 and `build_ff.bat`'s own comment are WRONG about this.** Both say ".gsc is
unaffected in practice — Plutonium runs raw `.gsc` out of `mod.iwd`". True for scripts that only exist
in `mod.iwd`; **not** true for one the zone also declares. `build_ff.bat` stages only `*.csc`, so this
file has never been refreshed or removed since the merge.

Right now the four colliding bodies are **identical** to the current ones (dumped and compared), so
the duplication is inert *today*. That is luck, not design: any future edit to those four functions in
`quality_of_life.gsc` silently competes with a July copy.

▶️ **THE FIX (its own round):** delete the `scripts/zm/zm_expanded.gsc` line from
`zone_source\mod_base.zone`, relink, boot once and confirm the four WARNING lines are gone.
🛑 `mod_base.zone` is regenerated by `build_ff.bat regen`, so the edit must either be re-applied after
a regen or `regen` must learn to drop the line. Decide that before touching it.
📝 While there: audit the whole zone for other declared-but-deleted scripts.

---

### 🟡 B-TITUSRELOAD — measured. BO2 HAS NO TITUS RELOAD AUDIO; the path is notetracks.

User, 2026-08-16: *"the titus still has no reload sound effect either."*

**Measured, and it rules out the obvious fix:**
- The audio dumper's 96 identifier files contain **exactly four** Titus payloads, all in the
  **singleplayer** bank `spl_code_post_gfx.all`: `wpn_dart_alert`, `wpn_titus_dart`,
  `titus_flip_down`, `titus_flip_up`. **No reload sound for this weapon exists in the game.**
- So reload foley must come from the animation's **notetracks**. And
  `notetrackSoundMap` is **empty in our def AND in Treyarch's own campaign def** (`titus6_sp`,
  dumped from `monsoon.ff`) — so ours is not missing anything the campaign has.
- Every sound field was diffed against the campaign def; the only differences are deliberate
  zombies-side choices (`fire*` → the explosive variant, `raise`/`pickup` → shoulder/GL foley).
- All three reload animations ARE shipped: `viewmodel_titus_mk_reload`,
  `viewmodel_titus_mk_reload_empty`, `viewmodel_titus_gl_reload_empty` (checked against mod.ff's
  2,065 xanims).

▶️ **NEXT STEP, bounded:** dump `viewmodel_titus_mk_reload` out of `mod.ff` and read the **notetrack
names** out of the binary, then check each against a loaded zombies bank. That names the aliases; a
missing one gets a `notetrackSoundMap` entry pointing at the generic `fly_*` row the campaign relies
on. Nothing was guessed and nothing was shipped.

📝 **Side finding, unrelated but real:** `titus6_zm` references three animations that are **NOT in
mod.ff** — `viewmodel_titus_gl_d2p_in`, `_d2p_loop`, `_d2p_out` (dive-to-prone). Harmless unless the
player dives to prone holding it; worth fixing with the same trip.

---

### 🔴 B-WHOWL — Winter's Howl fx: the CRLF fix was real but did NOT fix it. Offline checks EXHAUSTED.

v1.97.0 restored all 60 `.efx` to LF, byte-identical to the shipped working build. The user booted
v1.98.0 and reports the firing fx are **still wrong**, and look like the Thundergun's.

**Everything checkable offline now checks out, and each was measured this round:**

| check | result |
|---|---|
| the `.efx` is byte-identical to `Wonder_Weapons-T6ZM\wonder_wepons_zm\mod.iwd` | ✅ 60/60 |
| the effect actually loads at runtime | ✅ `console_zm.log:4527 Loaded fx: weapon/muzzleflashes/fx_freezegun_view` |
| the weapon def names the right fx | ✅ identical to the working port AND to `SRS_T5_WonderWeapons_portable` |
| is it secretly a copy of the thundergun's file? | ❌ no — different content, different materials, distinct md5 |
| do its materials resolve on TranZit? | ✅ all 7. (`chr_shock_hb1` is an `elemSpawnSound`, not a material) |
| does the thundergun's script play its fx on other weapons? | ❌ it filters on `getcurrentweapon()` |
| a full material audit of all 60 `.efx` | 6 missing of 56 — **none of them in any freezegun effect** (`gfx_fxt_thundergun_view_glow`, `gfx_fxt_smk_gen_vm`, and 4 water/blood ones) |

**What is left, and it needs the game:** the freezegun flash and the thundergun flash SHARE three of
their elements (`gfx_fxt_fx_distortion_heat`, `..._ring_warp`, `gfx_fxt_lensflare_diamond`). If the
freezegun-only elements are being dropped at runtime for a reason invisible to a file audit, what
remains IS the shared distortion pair — which would genuinely read as "the thundergun's effect".

▶️ Next: a `.testfx <name>` console command that plays a NAMED fx at the muzzle on demand, so the two
can be compared side by side in one boot. That distinguishes "the file is wrong" from "the wiring is
wrong" and is the only thing that will.
