...
- SYNCED TO: checkpoint **82** · mod version **1.99.80**
...
- **BUILT, AWAITING THE USER’S BOOT: v1.99.80** - the texture pack ships as **119 `<decimal ipak name-hash>.iwi`** files plus 2 by name (item 34); `mod.iwd` = 416 MB. 🛑 **v1.99.78 fixed a LOAD-TIME CRASH** - `is_headshot` / `get_base_weapon_name` had lost their backslashes in the v1.99.75 BETTER DEADSHOT probe, so no map could start. The v1.99.76 search-order probe was **deleted in v1.99.80**: its target `xenonbutton_a` is itself an ipak image, so a by-name file could never have overridden it and the probe could not answer anything.
